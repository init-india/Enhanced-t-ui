...
import ohi.andre.consolelauncher.commands.main.specif.ParamCommand;
...
public class Command {
    ...
    public String exec(ExecutePack info) throws Exception {
        ...
        if (cmd instanceof ParamCommand) {
            if (indexNotFound == 0) {
                return info.context.getString(R.string.param_missing);
            }
            ...
            if (nArgs < args.length || (mArgs == null)) {
    ...
    public int nextArg() {
        boolean useParamArgs = cmd instanceof ParamCommand;
        int[] args;

        if (useParamArgs) {
            if (!(mArgs[0] instanceof Param)) {
                args = new int[0];
            } else {
